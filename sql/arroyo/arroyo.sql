...
CREATE TABLE redis_block_ip_list (
    key_suffix TEXT NOT NULL,  -- Redis key 后缀部分：{host}:{ip}，完整 key = key_prefix + key_suffix
    host TEXT NOT NULL,
    ip TEXT NOT NULL,
    ...
    'target.key_column' = 'key_suffix',
    'target.ttl_secs' = '7200',  -- 固定2小时（7200秒）
    'json.timestamp_format' = 'RFC3339'
);
...
CREATE TABLE redis_suspicious_users (
    key_suffix TEXT NOT NULL,  -- Redis key 后缀部分：{host}:{user_id}，完整 key = key_prefix + key_suffix
    host TEXT NOT NULL,
    ...
    'target.key_column' = 'key_suffix',
    'target.ttl_secs' = '1800',  -- 固定30分钟（1800秒）
    'json.timestamp_format' = 'RFC3339'
);
...
CREATE TABLE redis_traffic_stats (
    key_suffix TEXT NOT NULL,  -- Redis key 后缀部分：{host}:{metric_type}:{metric_key}，完整 key = key_prefix + key_suffix
    host TEXT NOT NULL,
    ...
    'target.key_column' = 'key_suffix',
    'target.ttl_secs' = '300',  -- 固定5分钟（300秒）
    'json.timestamp_format' = 'RFC3339'
);
...
CREATE TABLE redis_rate_limit_config (
    key_suffix TEXT NOT NULL,  -- Redis key 后缀部分：{host}:{endpoint}，完整 key = key_prefix + key_suffix
    host TEXT NOT NULL,
    ...
    'target.key_column' = 'key_suffix',
    'target.ttl_secs' = '120',  -- 固定2分钟（120秒）
    'json.timestamp_format' = 'RFC3339'
);
...
CREATE TABLE redis_hot_urls (
    key_suffix TEXT NOT NULL,  -- Redis key 后缀部分：{host}:{url_path}，完整 key = key_prefix + key_suffix
    host TEXT NOT NULL,
    ...
    'target.key_column' = 'key_suffix',
    'target.ttl_secs' = '600',  -- 固定10分钟（600秒）
    'json.timestamp_format' = 'RFC3339'
);
...
SELECT 
    host || ':' || remote_addr as key_suffix,
    host,
    remote_addr as ip,
...
SELECT
    t.host || ':' || COALESCE(u.id::TEXT, t.user_key) as key_suffix,
    t.host,
...
SELECT
    host || ':total_requests:all' as key_suffix,
    host,
