...
async fn inner_fetch_trade(model: pay_order::Model, svc: &PayOrderService) -> anyhow::Result<()> {
    ...
    user_info::ActiveModel::add_expiration_days(&svc.db, model.user_id, model.level.days() as i64)
        .await
        .with_context(|| format!("add expiration days failed for order#{order_id}"))?;
